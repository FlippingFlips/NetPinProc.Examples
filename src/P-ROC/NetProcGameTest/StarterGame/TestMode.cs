...
namespace NetProcGameTest.StarterGame
{
    public class TestMode : Mode
    {
        ...
        private Dictionary<string, IDriver> _trap_switch_coils;

        public TestMode(GameController game, Dictionary<string, IDriver> drivers)
            : base(game, 90)
        {
            if (drivers != null)
                _trap_switch_coils = drivers;
            else
                _trap_switch_coils = new Dictionary<string, IDriver>();
        ...
        private void toggle_lamp(IDriver d)
        {
            if (d.State.State)
            {
                d.Disable();
